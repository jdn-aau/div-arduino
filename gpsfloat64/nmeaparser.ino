...
void
bloxCfgDisableAll (HardwareSerial & serPort)
{
...
void
getDegFrac (char *d, char *f, char *pkg)
{
  char *p = pkg;
  // deg part in degr
  while ('.' != *(p + 2))
...
nextCommaCopy (char *s, char *dst)
{
  char *ss = dst;
  while ((',' != *s) && ('*' != *s))
    {				// * is delimiter of last field bef checksum
      *dst = *s;
      s++;
      dst++;
    }
...
char
getNmeaPkg (HardwareSerial & ser, nmeaPkgTp * buf)
{
  int r;
...
int
activateNmeaType (unsigned char id)
{
...
System (Galileo)
  ...
  Indi Â ·a GL Globalnaya Navigazionnaya Sputnikovaya Sistema (GLONASS)
  ...
  COUNTRY GA European Global Navigation System (Galileo)
     ...
     Japan https:		//docs.novatel.com/OEM7/Content/Logs/GPGGA.htm
       ...
       W:West 6 GPS Quality indicator:0:Fix not valid 1:GPS fix 2:Differential
       GPS fix (DGNSS), SB$AS, OmniSTAR VBS, Beacon,
  RTX in GVBS mode 3:Not applicable 4:RTK Fixed, xFill 5:RTK Float,
  OmniSTAR XP / HP, Location RTK,
  RTX 6:INS Dead reckoning 7 Number of SVs (space vehicles:sats)
     in use,
       range from 00 through to 24 + 8 HDOP -
       Horizontal dilution of precision .9 Orthometric height (MSL reference)
     10 M:unit of measure for orthometric height is meters 11 Geoid separation 12 M:geoid separation measured in meters 13 Age of differential GPS data record, Type 1 or Type 9. Null field when DGPS is not used .14 Reference station ID, range 0000 to 4095. A null field when any reference station ID is selected and no corrections are received.See table below for a description of the field values .15 The checksum data, always begins with * ---------------------------------------------GLL GLL https:	//receiverhelp.trimble.com/alloy-gnss/en-us/NMEA-0183messages_GLL.html
  0 Message ID $GPGLL 1 Latitude in dd mm, mmmm format (0 - 7 decimal places)
     2 Direction of latitude N:North S:South
       3 Longitude in ddd mm, mmmm format (0 - 7 decimal places)
4 Direction of longitude E:East W:West
       5 UTC of position in hhmmss.ss format
       6 Status indicator:A:Data valid
       V:Data not valid
       This value is set to V (Data not valid) for all Mode Indicator
  values except A (Autonomous)
     and D (Differential)
     7 The checksum data,
       always begins with *
       Mode indicator:A:Autonomous mode D:Differential mode E:Estimated (dead
									 reckoning)
     mode M:Manual input mode S:Simulator mode N:Data not valid-- -- -----------------------------VTG VTG https:	//receiverhelp.trimble.com/alloy-gnss/en-us/NMEA-0183messages_VTG.html
